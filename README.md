...
O par que tiver `grafico` na configuração ganha também um gráfico embutido — só com a **EMA89** já carregada, a mesma linha que o cartão cita. RSI e ADX ficam de fora de propósito: os dois juntos poluem o embed, e quem quer um deles escolhe na hora, pelo próprio widget —, redesenhado quando o tema muda — o gráfico mora num iframe e o tema dele é escolhido na criação do widget, não por CSS. A Kraken não publica widget de embed próprio; o TradingView publica, e serve a série da **própria Kraken** sob o símbolo `KRAKEN:…` — é a mesma fonte do relatório, não uma segunda opinião. Se o script não carregar, fica no lugar um link para o gráfico completo e nada mais na página se perde.
