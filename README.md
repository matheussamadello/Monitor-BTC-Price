...
Não há regra por **horário**, de propósito. Quem quer tema escuro à noite já liga o agendamento automático do próprio sistema, e o `prefers-color-scheme` entrega isso de graça. Uma regra própria brigaria com quem escolheu claro deliberadamente, e faria a página mudar de cara sozinha conforme a hora de abrir — o que se lê como defeito, não como recurso. O tema claro **só redefine tokens de cor**: nenhuma regra de layout existe duas vezes, então os dois não têm como divergir de estrutura. Há um teste que compara os dois conjuntos de tokens e falha se alguém acrescentar uma cor no escuro e esquecer do claro — senão o tema claro herdaria uma cor de fundo escuro em silêncio. A faixa dos botões `Diário`/`Semanal` e o container do gráfico usam tokens à parte — `--tv-barra`, `--tv-fundo` e `--tv-linha` —, porque o embed do TradingView pinta a própria moldura no cinza do tema dele, que não é o azul-noite do resto da página, e três tons de escuro empilhados leem como defeito de renderização. **A cor veio de conta-gotas na tela, não de dedução.** Duas tentativas erraram por deduzir da paleta clássica do TradingView: `#131722`, o fundo do gráfico, mexeu `(3, −1, −7)` no pixel pintado e foi invisível; `#1e222d`, a barra de ferramentas, ficou claro demais. O widget que carrega hoje usa o tema escuro novo, `#0f0f0f`, quase preto e neutro, que não está naquela paleta. O tema claro tinha o mesmo erro e demorou mais a aparecer: a faixa ficou em `#f1f3f6`, a barra de ferramentas da paleta clássica, enquanto o widget recebia `#ffffff` como fundo do gráfico — as duas superfícies discordavam entre si antes mesmo do TradingView entrar na conta. Medido, o embed claro é branco puro — e por isso **o branco puro passou a ser exclusivo dele**: os painéis usam `#fbfcfe`, um branco levemente azulado, para a escada `--bg` → `--painel2` → `--painel` → embed ter quatro degraus em vez de terminar em duas chapas brancas iguais. O hex vai também ao widget a partir da mesma constante que alimenta o CSS, para os dois não poderem divergir. Os botões `Diário`/`Semanal` seguem a mesma lógica e são **cinza**, não o azul da página: eles moram dentro do bloco do gráfico, que é território do widget e neutro nos dois temas. As duas cores se invertem entre os temas — botão `#d3d3d3` com texto `#777777` no escuro, botão `#777777` com texto `#d3d3d3` no claro —, então são dois tokens e não quatro. Como os dois botões ficam na mesma cor, o marcado se distingue por um **anel** na cor do próprio texto, em vez de um terceiro cinza. Vale saber que `#d3d3d3` sobre `#777777` dá 3,0:1 de contraste, abaixo dos 4,5:1 recomendados para texto de 11px: foi uma escolha deliberada de aparência.
