...
Para **você**: um cartão por par com o resumo dos dois timeframes — último fechamento, lado e distância da EMA89 **em porcentagem**, RSI, ADX com DI+/DI−, estrutura, situação dos níveis manuais e ATR **em porcentagem** —, mais os alertas técnicos como etiquetas.

O cartão mostra porcentagem onde o relatório mostra ATR, e isso é só apresentação. `0,0418` não diz nada de relance — é muito ou pouco dependendo do par —, enquanto `0,81%` se lê na hora. O relatório continua publicando `atr14`, `atr14_pct` e `distancia_ema89_fechada_atr`, e o prompt continua decidindo pela versão em ATR: 0,25 ATR de margem para a travessia semanal, 1,0 ATR para a corroboração da perda diária. ATR é a unidade certa para comparar pares de volatilidades diferentes; porcentagem é a que se lê de relance. A distância usa a própria EMA como denominador, a mesma convenção do `distancia_ema89_pct` — só que com o fechamento no lugar do preço vivo, para o cartão não voltar a misturar vela fechada com vela em formação. O que está em `deterioracao_tendencia` sai em vermelho; o resto, em azul. O carimbo de tempo no topo calcula sozinho, no navegador, há quanto tempo o relatório foi gerado, e muda de cor a partir de 90 minutos.
