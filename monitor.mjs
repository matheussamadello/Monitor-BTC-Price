// ============================================================
// Monitor BTC — snapshot tecnico diario (Kraken)
...
// No monitor de XMR os numeros dos gatilhos estavam repetidos dentro de
// avaliarGatilhos; aqui existem uma vez so, para nao ficar meio
// atualizado depois de uma revisao de niveis.
//
// Os valores abaixo sao um PONTO DE PARTIDA baseado na regiao em que o
...
const PAIRS = [
  {
    key: "usd",
    label: "BTC/USD",
    // Na Kraken o Bitcoin e' XBT. O par de consulta e' XBTUSD e a chave
    // devolvida dentro de result vem como XXBTZUSD — parseKraken pega a
    // primeira chave diferente de "last", entao isso resolve sozinho.
    par: "XBTUSD",
    // Simbolo do grafico publicado na pagina. A Kraken nao tem widget
    // de embed proprio; o TradingView tem, e serve a serie da PROPRIA
    // Kraken sob este simbolo -- e' a mesma fonte do relatorio, nao uma
    // segunda opiniao. Par sem este campo simplesmente nao ganha grafico.
    grafico: "KRAKEN:XBTUSD",
    graficoNota:
      "Série da Kraken (KRAKEN:XBTUSD) desenhada pelo TradingView — a mesma fonte do relatório acima.",
    dec: 2,
    niveis: NIVEIS_USD,
  },
];
...
// repositorio so.
const AGENTE_HTTP = "btc-monitor/1.0";
...
// relativa: um limiar absoluto de corpo seria arbitrario, e o BTC muda
// de ordem de grandeza ao longo dos anos — a mediana acompanha sozinha.
...
// Em ATR, como todo o resto do projeto. Eram 0,5% e 3% fixos, iguais
// para qualquer par -- e um percentual fixo vale coisas diferentes em
// cada ativo: 0,5% valia 0,077 ATR no XMR/USD e 0,592 ATR no USDT/BRL,
// 7,7x de diferenca. A janela de reteste era dez vezes mais estreita
// que uma zona automatica num par e quase do tamanho de uma zona
// inteira no outro, e o reset do USDT/BRL (3,55 ATR) era maior que os
// 3 ATR em que os niveis ja sao declarados obsoletos, entao o ciclo
// praticamente nunca reiniciava. "reteste_confirmado" queria dizer
// coisas diferentes em cada par.
...
// antigos, para o comportamento nao mudar nesse canto.
const RETEST_TOLERANCIA_PCT_FALLBACK = 0.5;
const RETEST_RESET_PCT_FALLBACK = 3;
...
const TITULO_PAGINA = "Monitor BTC";
...
const TITULO_ALERTA = "ALERTA BTC";
...
:root{
  ...
  --tv-btn:#e6e6e6; --tv-btn-txt:#3a3a3a;
...
html[data-tema="claro"]{
  ...
  --tv-btn:#3a3a3a; --tv-btn-txt:#e6e6e6;
}
...
/* Cinza, e nao o azul do resto da pagina: estes botoes moram DENTRO
   ...
   AS QUATRO CORES SAO ESCOLHA DO DONO DO MONITOR, pedidas uma a uma:
     ...
     noite  marcado #e6e6e6        #e6e6e6    #3a3a3a
     claro  solto   transparente   #3a3a3a    #3a3a3a
     claro  marcado #3a3a3a        #3a3a3a    #e6e6e6
   ...
   CONTRASTE MEDIDO: 15,4:1 no solto do escuro, 11,4:1 no solto do
   claro e 9,1:1 nos dois marcados.

   O par foi aberto aos poucos, a pedido do dono do monitor, partindo
   de #d3d3d3/#777777, que dava 3,0:1 entre si. A abertura ficou
   ASSIMETRICA de proposito: toda a separacao veio do cinza escuro,
   que desceu de #777777 a #3a3a3a, enquanto o claro voltou a
   #e6e6e6. E' o lado que rende -- ele carrega o texto do solto no
   tema claro, onde compete com o branco da faixa, e la o ganho foi de
   4,5:1 para 11,4:1. Escurecer mais o claro daria pouco: la o texto
   compete com o quase-preto do embed, e ja estava folgado em 15:1.

   Os valores sao escolha de aparencia, nao derivados de regra. Os
   marcos tecnicos -- 4,5:1 para texto de 11px e 7:1 do nivel AAA --
   ficaram cumpridos nas quatro medidas, mas como consequencia e nao
   como criterio. */
